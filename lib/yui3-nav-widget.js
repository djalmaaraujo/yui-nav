...
YUI().use(modules, function (Y) {
	...
	var NavWidget = Y.Base.create("nav-widget", Y.Widget, [], {
		...
		/**
		 * Grab delegated ON node events.
		 * 
		 */
		_delegateEvents: function (obj, element) {
			var onEvents = obj.on;
			if (Lang.isObject(onEvents, true)) {
				Y.Object.each(onEvents, function (fn, event) {
					Node.delegate(event, fn, 'li');
				});
			}
		},
