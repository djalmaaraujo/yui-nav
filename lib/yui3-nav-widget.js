...
YUI().use(modules, function (Y) {
	...
	var NavWidget = Y.Base.create("nav-widget", Y.Widget, [], {
		...
		_attachNavOptions: function () {
			var dataSource 	= this.dataSource.urls;
		...
		// 	
		// 		li = Node.create('<li/>')
		// 		a = Node.create('<a/>').set('href', index.url).set('text', index.text);
		// 		li.append(a);
		// 
		// 		if (Y.Lang.isObject(index.urls, true)) {
		// 			var sub = instance._recursiveAttachJson(index.urls, instance);
		// 			li.append(sub);
		// 		}
		// 		
		// 		ul.append(li);
		...
		_recursiveAttachJson: function (container, instance) {
			...
			Y.Array.each(container, function (index, value) {
				...
				li = '<li>';
				li += '<a href="' + index.url + '">' + index.text + '</a>';
		
				if (Y.Lang.isObject(index.urls, true)) {
					li += instance._recursiveAttachJson(index.urls, instance);
				}
